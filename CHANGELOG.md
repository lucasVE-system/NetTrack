...
- **Fix (packaging)**: `build.py` now bundles the `static/` folder into the exe — packaged builds were missing `scan.js`/`topology.js`, breaking scan and topology buttons in the desktop app.
...
- **Tests**: suite expanded from 6 to 80 tests, now covering `scanner.py`, `topology.py` (LLDP parser, device-type inference, graph builder), `dns_sniffer.py` (DNS parser, categorisation, persistence), release signing and the new SNMP config validation.
